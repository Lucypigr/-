(() => {
  const G=window.GRIMSKILL, X=window.GRIMWEAPON;
  if(!G||!X) return;
  ...
  function dotTick(rt,dt){
    for(let i=0;i<rt.enemies.length;i++){
      const e=rt.enemies[i];if(e.hp<=0)continue;
      let dot=0;
  ...
  function targetModifier(rt,i,w){
    const e=rt.enemies[i];let mult=1;
    if(e.shock>0)mult*=1+(rt.build.shock||0);
    if(w.tags?.includes('physical')&&e.armorBreak)mult*=1+e.armorBreak*.06;
    return mult;
  }
  function applyTrait(rt,i,w,baseDamage){
    const e=rt.enemies[i],tr=w.trait||{},kind=weaponFxKind(w);let dmg=baseDamage*targetModifier(rt,i,w),crit=false,label='';
    if(tr.id==='iaiCrit'&&game.rng()<tr.critChance){dmg*=tr.critMult;crit=true;label='暴擊';}
    if(tr.id==='precision'){
      ...
      dmg*=1+(rt.precisionStacks||0)*tr.stackDamage;label=`精準 ${rt.precisionStacks||0}/${tr.maxStacks}`;
    }
    if(tr.id==='focus'){
      if(rt.focusTarget===i)rt.focusStacks=Math.min(tr.maxStacks,(rt.focusStacks||0)+1);else{rt.focusTarget=i;rt.focusStacks=0;}
      dmg*=1+(rt.focusStacks||0)*tr.stackDamage;label=`聚焦 ${rt.focusStacks||0}/${tr.maxStacks}`;
    }
    rawHit(rt,i,dmg,kind,crit?'暴擊！':label);
    if(tr.id==='stun'&&e.hp>0&&game.rng()<tr.chance){const d=rt.enemyId==='boss'?tr.bossDuration:tr.duration;e.stunnedUntil=Math.max(e.stunnedUntil||0,rt.time+d);label=`暈眩 ${d.toFixed(1)}s`;}
    if(tr.id==='armorBreak'&&e.hp>0){e.armorBreak=Math.min(tr.maxStacks,(e.armorBreak||0)+1);label=`碎甲 ${e.armorBreak}/${tr.maxStacks}`;}
    if(tr.id==='bleed'&&e.hp>0&&game.rng()<tr.chance){e.bleeds ||= [];if(e.bleeds.length>=tr.maxStacks)e.bleeds.shift();e.bleeds.push({until:rt.time+tr.duration,dps:rt.build.damage*tr.dpsFactor});label=`流血 ${e.bleeds.length}/${tr.maxStacks}`;}
    if(tr.id==='corrosion'&&e.hp>0){e.poisons ||= [];if(e.poisons.length>=tr.maxStacks)e.poisons.shift();e.poisons.push({until:rt.time+tr.duration,dps:rt.build.damage*tr.dpsFactor});label=`毒素 ${e.poisons.length}/${tr.maxStacks}`;}
    if(tr.id==='frost'&&e.hp>0){e.frost=(e.frost||0)+tr.buildup;if(e.frost>=100){e.frost=0;const d=rt.enemyId==='boss'?tr.bossFreeze:tr.freeze;e.stunnedUntil=Math.max(e.stunnedUntil||0,rt.time+d);label=`冰凍 ${d.toFixed(1)}s`;}else label=`寒霜 ${Math.round(e.frost)}%`;}
    ...
    if(label&&e.hp>0){const st=document.querySelector(`.enemy-card[data-enemy-index="${i}"] .battle-status`);if(st)st.textContent=label;}
    return dmg;
  }
  function attackOnce(rt,w,castIndex){
    ...
    const base=rt.build.damage*(fury?1.15:1)*(charged?w.trait.mult:1);
    if(charged){$('#battlePlayerStatus').textContent='蓄力射擊 · 220%';stageShake('heavy');}
    const form=X.attackForm(w),volley=form==='projectile'?Math.min(7,rt.build.projectiles):1;
    for(let v=0;v<volley;v++)setTimeout(()=>castVfx(w,rt.build,v,volley,aliveNow[v%aliveNow.length]||0),v*40);

    if(w.trait?.id==='spearPierce'){
      aliveNow.slice(0,1+w.trait.extraTargets).forEach((i,n)=>{if(n>0)setTimeout(()=>spawnSlash(i,false),n*55);applyTrait(rt,i,w,base);});return;
    }
    const shots=form==='projectile'?rt.build.projectiles:1;
  ...
  async function liveBattleV27(c){
    ...
    const cls=X.damageClass(w),form=X.attackForm(w),tr=w.trait;
    $('#battleWeaponLine').innerHTML=`<b>${skillName(w)}</b> · 傷害 ${b.damage.toFixed(1)} · ${cls} · ${form}${form==='投射物'?` ×${b.projectiles}`:''} · ${b.rate.toFixed(2)}s${tr?`<br><span class="weapon-trait-battle">${tr.name}：${tr.desc}</span>`:''}`;
    ...
    while(rt.playerHp>0&&alive(rt).length&&rt.time<45){
      ...
      if(rt.time>=nextE&&alive(rt).length){
        ...
        if(attackers.length){let dmg=e.damage*Math.min(3,attackers.length);dmg*=b.equipmentBonuses?.damageTakenMult||1;dmg*=b.auraDamageTakenMult||1;
  ...
  playBattle=async c=>c?.interactive?liveBattleV27(c):previousPlay(c);
